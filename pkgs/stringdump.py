    ...
    def __removeBlackListStrings(self, allStrings):
        ...
        if regBlackList:
            ...
            for s in finalStringList:
                if any(regex.search(s) for regex in compiledRegexes):
                    regmatchList.append(s)

            if regmatchList:
                regmatchSet = set(regmatchList)
                finalStringList = [s for s in finalStringList if s not in regmatchSet]
